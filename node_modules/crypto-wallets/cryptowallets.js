module.exports = {
	...
	*	Returns true if the private key is the correct private key of the given address.
	*
	*	Note:	IOTA private keys are an HD seed that can generate an infinite number of addresses in 3 different security levels.
	*			This function only checks the first 5 addresses at each security level, so if the given address is generated by this private key, but
	*			it has an index >4, this will return false.
	*
	*/
	verifyIOTAPrivateKey: async function(privateKey, address){
		try{
			...
			for(var i = 1; i <= 3; i++){
				const iota = this._initIOTA();
				var addrs = await new Promise(function(resolve, reject){
					iota.api.getNewAddress(privateKey, {
						index: 0,
						checksum: true,
						total: numAddresses,
					...
					, function(err, addr){resolve(addr)});
				});
